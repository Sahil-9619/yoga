  ...
  try {
    ...
    if (!name || !email || !phone || !workshopId) {
      return res.status(400).json({
        success: false,
        message: "Name, email, phone and workshopId are required.",
      });
    }
    ...
    (async () => {
      try {
        ...
        const bookingDetails = {
          workshopTitle:
            workshopTitle || (workshop ? workshop.title : "Custom Session"),
...
const getMyBookings = async (req, res) => {
  try {
    ...
    const bookings = await Booking.findAll({
      where: { userId },
      include: [
        { model: Workshop, attributes: ["title", "date", "mode", "time"] },
      ],
...
const sendOtp = async (req, res) => {
  try {
    ...
    if (mailResult.simulated) {
      console.warn(
        "SMTP credentials not configured. OTP generated but not sent. OTP:",
        otp,
      );
      return res.status(200).json({
        success: true,
        message: "SMTP not configured, check console for OTP",
      });
    }
